...
import type {Category, Api, ApiResponse} from '../../types/api';
...
function fromApi(a: Api): FormState {
    return {
        name: a.name,
        description: a.description,
        url: a.url,
        categoryId: a.categoryId,
        isHttps: String(a.isHttps),
        isFree: String(a.isFree),
        authType: a.authType,
        corsStatus: a.corsStatus,
    };
}

export function AdminApiFormPage() {
    ...
    useEffect(() => {
        ...
        api.get<ApiResponse<{ api: Api }>>(`/catalog-api/${id}`)
            .then(res => setForm(fromApi(res.data.data.api)))
    ...
    async function handleSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
        ...
        try {
            if (isEdit) {
                await api.patch(`/catalog-api/${id}`, body);
            } else {
                await api.post('/catalog-api', body);
            }
            navigate('/admin/apis');
        } catch (err) {
                ...
                <div className="max-w-7xl mx-auto px-6 py-9 pb-20">
                    <div className="max-w-xl">
                        ...
                        <p className="text-sm text-slate-500 mb-8">
                            {isEdit ? 'Update the API entry in the catalog.' : 'Add a new API directly to the catalog.'}
                        </p>
                        ...
                        {isLoading ? (
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                <div>
                                    ...
                                    <input className={inputCls} type="text" value={form.name}
                                           onChange={e => set('name', e.target.value)}
                                           placeholder="e.g. OpenWeather API" required disabled={isSubmitting}/>
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea className={inputCls} value={form.description}
                                              ...
                                              placeholder="What does this API do?" rows={3} required
                                              disabled={isSubmitting} style={{resize: 'vertical'}}/>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input className={inputCls} type="url" value={form.url}
                                           ...
                                           placeholder="https://api.example.com" required disabled={isSubmitting}/>
                                </div>

                                <div>
                                    <label className={labelCls}>Category</label>
                                    <select className={selectCls} value={form.categoryId}
                                ...
                                <div className="flex gap-3 pt-1">
                                    <button type="submit" disabled={isSubmitting}
                                        ...
                                        {isSubmitting ? 'Saving…' : isEdit ? 'Save changes' : 'Create API'}
                                    </button>
